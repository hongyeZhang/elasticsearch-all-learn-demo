...
public class TaxonomyDemo {
    public void createIndex() throws Exception {
        ...
        // DirectoryTaxonomyWriter写入Taxonomy索引
        Directory taxioDirectory = FSDirectory.open(Paths.get("D:/Lucene/taxo"));
        DirectoryTaxonomyWriter taxoWriter = new DirectoryTaxonomyWriter(taxioDirectory);

        // facet配置，类似IndexWriterConfig
    ...
    public void facetIndex() throws IOException {
        Directory directory = FSDirectory.open(Paths.get("D:/Lucene/index"));
        DirectoryReader indexReader = DirectoryReader.open(directory);
        IndexSearcher searcher = new IndexSearcher(indexReader);
        // 分类读取
        Directory taxoDirectory = FSDirectory.open(Paths.get("D:/Lucene/taxo"));
        // 分类读取
        TaxonomyReader taxoReader = new DirectoryTaxonomyReader(taxoDirectory);
        FacetsConfig config = new FacetsConfig();
        // Collector主要用于从搜索中收集原始结果，并进行排序或对结果进行过滤，整理等操作
    ...
    public static void main(String[] args) {
        TaxonomyDemo td = new TaxonomyDemo();
        try {
            // td.createIndex();
            td.facetIndex();
        } catch (Exception e) {
